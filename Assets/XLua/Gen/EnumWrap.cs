...
namespace XLua.CSObjectWrap
{
    ...
    public class MessageViewTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(MessageViewType), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(MessageViewType), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(MessageViewType), L, null, 4, 0, 0);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Ok", MessageViewType.Ok);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "OkAndCancel", MessageViewType.OkAndCancel);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "UnderlyingSystemType", typeof(MessageViewType));
			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(MessageViewType), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            {
                translator.PushMessageViewType(L, (MessageViewType)LuaAPI.xlua_tointeger(L, 1));
            }
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
			    if (LuaAPI.xlua_is_eq_str(L, 1, "Ok"))
                {
                    translator.PushMessageViewType(L, MessageViewType.Ok);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "OkAndCancel"))
                {
                    translator.PushMessageViewType(L, MessageViewType.OkAndCancel);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for MessageViewType!");
                }
            }
            else
            {
                return LuaAPI.luaL_error(L, "invalid lua type for MessageViewType! Expect number or string, got + " + lua_type);
            }
