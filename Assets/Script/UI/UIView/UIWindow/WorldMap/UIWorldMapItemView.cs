...
public class UIWorldMapItemView : UISubViewBase
{
    ...
    public void SetUI(TransferData data, Action<int> onWorldMapItemClick)
    {
      ...
      //  imgIco.overrideSprite = GameUtil.LoadWorldMapIco(data.GetValue<string>(ConstDefine.WorldMapIco));

        AssetBundleMgr.Instance.LoadOrDownload<Texture2D>(string.Format("Download/Source/UISource/UI/WorldMap/{0}.assetbundle", picName), picName, (Texture2D obj) =>
        {
            if (obj == null) return;
