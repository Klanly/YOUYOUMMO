...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderXLuaTestPedding
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<MessageViewType>(translator.PushMessageViewType, translator.Get, translator.UpdateMessageViewType);
        ...
        int MessageViewType_TypeID = -1;
		int MessageViewType_EnumRef = -1;
        
        public void PushMessageViewType(RealStatePtr L, MessageViewType val)
        {
            if (MessageViewType_TypeID == -1)
            {
			    bool is_first;
                MessageViewType_TypeID = getTypeId(L, typeof(MessageViewType), out is_first);
				
				if (MessageViewType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(MessageViewType));
				    MessageViewType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, MessageViewType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, MessageViewType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for MessageViewType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, MessageViewType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out MessageViewType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != MessageViewType_TypeID)
				{
				    throw new Exception("invalid userdata for MessageViewType");
				}
                ...
                {
                    throw new Exception("unpack fail for MessageViewType");
                }
				val = (MessageViewType)e;
                
            }
            else
            {
                val = (MessageViewType)objectCasters.GetCaster(typeof(MessageViewType))(L, index, null);
            }
        }
		
        public void UpdateMessageViewType(RealStatePtr L, int index, MessageViewType val)
        {
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != MessageViewType_TypeID)
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for MessageViewType ,value="+val);
                }
            }
            else
            {
	...
	public partial class StaticLuaCallbacks
    {
	    static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(MessageViewType[]))
			{
			    MessageViewType[] array = obj as MessageViewType[];
				translator.PushMessageViewType(L, array[index]);
				return true;
			}
